...
void Run::update(Brain *brn) {
  ...
  for (i=0; i<nlocal; i++) {
    ...
    agent[neu][i][1] -= brn->dn * agent[ast][i][0] * agent[neu][i][0];
...
void Run::derivatives(Brain *brn) {
  ...
  // set derivatives of all voxels to zero, set gradients to zero
  for (ag_id=0; ag_id<num_agents; ag_id++)
    for (i=0; i<nall; i++) {
      agent[ag_id][i][1] = 0.0;
      grad[ag_id][i][0] = grad[ag_id][i][1] = grad[ag_id][i][2] = 0.0;
    }
...
void Run::update(Brain *brn) {
  ...
  for (i=0; i<nlocal; i++) {
    ...
    // sAb
    agent[sAb][i][1] += agent[neu][i][0] * agent[act][i][0] * 1.0e3
                      - dum
                      - brn->ds * agent[mic][i][0] * agent[sAb][i][0];
    // fAb
    agent[fAb][i][1] += dum
